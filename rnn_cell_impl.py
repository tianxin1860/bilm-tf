  ...
  def call(self, inputs, state):
    ...
    for i, cell in enumerate(self._cells):
      with vs.variable_scope("cell_%d" % i):
        ...
        cur_inp_, new_state = cell(cur_inp, cur_state)
        new_states.append(new_state)
        #cur_inps.append(cur_inp)
        cur_inps.append(cur_inp_)
        cur_inp = cur_inp_
        (c_prev, m_prev) = cur_state
        c_prevs.append(c_prev)
        m_prevs.append(m_prev)

    new_states = (tuple(new_states) if self._state_is_tuple else
                  array_ops.concat(new_states, 1))

    cur_inps = array_ops.concat(cur_inps + m_prevs + c_prevs, -1)

    return cur_inps, new_states
